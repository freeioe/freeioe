...
local function make_packet_subscribe(args)
	...
	for i, subscription in ipairs(args.subscriptions) do
		...
		if subscription.retain_handling then
			so = so | (subscription.retain_handling << 4) -- set Bit 4 and 5
		end
...
local function parse_packet_subscribe(ptype, flags, input)
	...
	while input.available > 0 do
		...
		subscriptions[#subscriptions + 1] = {
			topic = topic_filter,
			qos = (subscription_options & 0x3),
			no_local = (subscription_options & 0x4) ~= 0,
			retain_as_published = (subscription_options & 0x8) ~= 0, -- Retain As Published
			retain_handling = (subscription_options >> 4) & 0x3, -- Retain Handling
		}
	end
