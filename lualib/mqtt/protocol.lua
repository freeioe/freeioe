...
function protocol.make_uint16(val)
	...
	return str_char(val >> 8, (val & 0xFF))
end
...
function protocol.make_uint32(val)
	...
	return str_char(val >> 24, (val >> 16) & 0xFF, (val >> 8) & 0xFF, val & 0xFF)
end
...
function protocol.parse_uint32(read_func)
	...
	if _VERSION < "Lua 5.3" then
		return byte1 * (2 ^ 24) + (byte2 << 16) + (byte3 << 8) + byte4
	else
...
function protocol.make_header(ptype, flags, len)
	local byte1 = (ptype << 4) | (flags & 0x0F)
...
function protocol.parse_packet_connect_input(input, version)
	...
	if (connect_flags & 0x1) ~= 0 then
		return false, "reserved 1st bit in connect flags are set"
	end
	local clean = ((connect_flags & 0x2) ~= 0)
	local will = ((connect_flags & 0x4) ~= 0)
	local will_qos = ((connect_flags & 3) >> 0x3)
	local will_retain = ((connect_flags & 0x20) ~= 0)
	local password_flag = ((connect_flags & 0x40) ~= 0)
	local username_flag = ((connect_flags & 0x80) ~= 0)
